  ...
  return (
    <div>
      <div className="mb-10">
        ...
        <h1 className="text-3xl font-bold text-white mb-3" style={{ fontFamily: "'Space Grotesk', sans-serif", letterSpacing: '-0.03em' }}>Providers</h1>
        <p className="text-neutral-500 text-[15px]">How to use <code className="font-mono text-[0.9em]">observe()</code> with each supported provider.</p>
      </div>
...
const traced = observe(client, Provider.OpenRouter);`}</code></pre>
          }
          py={
            <pre className="bg-[#111111] border border-neutral-800 p-4 font-mono text-sm text-neutral-400 overflow-x-auto"><code>{`traced = observe(client, Provider.OPENAI)
traced = observe(client, Provider.ANTHROPIC)
traced = observe(client, Provider.OPENROUTER)`}</code></pre>
          }
        />
      ...
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-white mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>OpenAI</h2>
        <CodeTabs
          ts={
...
for await (const chunk of stream) {
  process.stdout.write(chunk.choices[0]?.delta?.content ?? '');
          ...
          py={
            <pre className="bg-[#111111] border border-neutral-800 p-4 font-mono text-sm text-neutral-400 overflow-x-auto"><code>{`from openai import OpenAI
...
for chunk in stream:
  print(chunk.choices[0].delta.content or "", end="")`}</code></pre>
      ...
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-white mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Anthropic</h2>
        <CodeTabs
          ts={
            <pre className="bg-[#111111] border border-neutral-800 p-4 font-mono text-sm text-neutral-400 overflow-x-auto"><code>{`import Anthropic from '@anthropic-ai/sdk';
import { observe, Provider } from '@pulse/sdk';
...
for await (const event of stream) {
  if (event.type === 'content_block_delta') {
    process.stdout.write(event.delta.text);
          ...
          py={
            <pre className="bg-[#111111] border border-neutral-800 p-4 font-mono text-sm text-neutral-400 overflow-x-auto"><code>{`from anthropic import Anthropic
from pulse_sdk import observe, Provider
  ...
  for text in stream.text_stream:
    print(text, end="")`}</code></pre>
        ...
        <div className="mt-4 border border-neutral-800 bg-[#111111] p-4 text-sm text-neutral-500">
          <strong className="text-neutral-300">Stop reason mapping.</strong> Anthropic stop reasons are normalized: <code className="font-mono text-[0.9em]">end_turn</code> &rarr; <code className="font-mono text-[0.9em]">stop</code>, <code className="font-mono text-[0.9em]">max_tokens</code> &rarr; <code className="font-mono text-[0.9em]">length</code>, <code className="font-mono text-[0.9em]">stop_sequence</code> &rarr; <code className="font-mono text-[0.9em]">stop</code>, <code className="font-mono text-[0.9em]">tool_use</code> &rarr; <code className="font-mono text-[0.9em]">tool_calls</code>.
        </div>
      </section>
...
res = client.chat.completions.create(
  ...
  messages=[{"role": "user", "content": "Hello"}],
      ...
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-white mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Pricing</h2>
        <p className="text-sm text-neutral-500 mb-4">The SDK calculates cost automatically for known models. Pricing is built in for:</p>
          ...
          <tbody>
            {[
              ...
              <tr key={model} className="border-b border-neutral-800">
                <td className="p-3"><code className="font-mono text-sm text-neutral-400">{model}</code></td>
        ...
        <p className="text-sm text-neutral-500 mt-3">Model aliases (e.g. <code className="font-mono text-[0.9em]">gpt-4o-2024-11-20</code>) are resolved to their base model for pricing. Unknown models report <code className="font-mono text-[0.9em]">null</code> cost.</p>
      </section>
